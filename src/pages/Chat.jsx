...
const Chat = () => {
    ...
    const findAndSetChannel = () => { //해당 유저의 팀과 팀채널을 찾아서 dispatch로 현재 채널을 설정해줌.
    ...
    useEffect(() => { //페이지 마운트 시 채널 전부를 가져와서 모든 채널 정보를 setChannels 해준다
    ...
    useEffect(() => { //페이지 마운트 시 기존 채널에 있던 메세지 정보를 들고와서 setMessages 해준다
        async function getMessages() {
            const snapShot = await get(
                child(ref(getDatabase()), "messages/" + currentChennel.id)
            );
    ...
    useEffect(() => {
        const sorted = query(
            ref(getDatabase(), "messages/" + currentChennel.id),
