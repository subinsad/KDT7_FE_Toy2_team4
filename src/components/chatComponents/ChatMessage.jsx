  ...
  return (
    <ListItem>
      ...
      <Grid container sx={{ ml: 2 }}>
        <Grid item xs={12} sx={{ display: "flex", justifyContent: "left" }} className={message.user.id === uid && "me"}>
          <ListItemText
            sx={{ display: "flex", marginTop: "-4px" }}
            primary={message.user.name}
            primaryTypographyProps={{
              fontWeight: "bold",
              color: message.user.id === uid ? "orange" : "black",
            }}
        ...
        <Grid item xs={12}>
          <ListItemText align="left" xs={{ wordBreak: "break-all", marginTop: "-4px" }} primary={message.content} />
        </Grid>
      </Grid>
    </ListItem>
  );
